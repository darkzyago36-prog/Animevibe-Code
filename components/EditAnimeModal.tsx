...
interface EditAnimeModalProps {
  isOpen: boolean;
  onClose: () => void;
  onEdit: (id: number, anime: Omit<Anime, 'id' | 'slug' | 'novo'>) => void;
  animeToEdit: Anime | null;
}

export function EditAnimeModal({ isOpen, onClose, onEdit, animeToEdit }: EditAnimeModalProps) {
  const [titulos, setTitulos] = useState(animeToEdit?.titulos || "");
  const [genero, setGenero] = useState(animeToEdit?.genero || "");
  const [nota, setNota] = useState<number | "">(animeToEdit?.nota || 10);
  const [capa_url, setCapaUrl] = useState(animeToEdit?.capa_url || "");
  const [tags, setTags] = useState<string[]>(
    animeToEdit?.tags ? [...animeToEdit.tags, ...(animeToEdit.destaque ? ["DESTAQUE"] : [])] : (animeToEdit?.destaque ? ["DESTAQUE"] : [])
  );
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const updatedData = {
      ...
      nota: Number(nota) || 0,
      genero,
      capa_url: capa_url || "https://picsum.photos/seed/anime/300/450",
      destaque: tags.includes("DESTAQUE"),
      tags: tags.filter(t => t !== "DESTAQUE"),
    };

    setTimeout(() => {
      onEdit(animeToEdit.id, updatedData as any);
            ...
            <div className="p-4 sm:p-5 overflow-y-auto flex-1">
              ...
              <div className="mb-3">
                ...
                <p className="text-gray-400 font-body text-sm">
                  Altere os detalhes do anime selecionado.
                </p>
              </div>

              <form onSubmit={handleSubmit} className="space-y-3">
                <div>
                  <label htmlFor="edit-titulos" className="block text-xs font-bold uppercase tracking-wider text-gray-400 mb-2 font-body">Título</label>
                  <input
                    type="text"
                    id="edit-titulos"
                    required
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label htmlFor="edit-genero" className="block text-xs font-bold uppercase tracking-wider text-gray-400 mb-2 font-body">Gênero</label>
                    <input
                      type="text"
                      id="edit-genero"
                      required
                  ...
                  <div>
                    <label htmlFor="edit-nota" className="block text-xs font-bold uppercase tracking-wider text-gray-400 mb-2 font-body">Nota</label>
                    <input
                      type="number"
                      id="edit-nota"
                      required
                ...
                <div>
                  <label htmlFor="edit-capa_url" className="block text-xs font-bold uppercase tracking-wider text-gray-400 mb-2 font-body">Capa do Anime</label>
